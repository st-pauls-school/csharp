...
namespace SectionD
{
  public class SectionDProgram
  {
    public static void SectionDMain(string[] args)
    {
      Simulation Sim;
      int MenuOption;
      int LandscapeSize;
  ...
  class Simulation
  {
    ...
    private void CreateLandscapeAndAnimals(int InitialWarrenCount, int InitialFoxCount, bool FixedInitialLocations)
    {
      for (int x = 0; x < LandscapeSize; x++)
      {
        for (int y = 0; y < LandscapeSize; y++)
        {
          Landscape[x, y] = new Location((x == 5 || y == 2) ? 'R' : 'L');
        }
      }
    ...
    private bool CheckIfPathCrossesRiver(int x1, int y1, int x2, int y2) {
      for(int i = x1 <= x2 ? x1 : x2; i <= (x1 <= x2 ? x2 : x1); i ++)
        if(Landscape[i, y1].Terrain == 'R')
          return true;
      for(int i = y1 <= y2 ? y1 : y2; i <= (y1 <= y2 ? y2 : y1); i ++)
        if(Landscape[x1, i].Terrain == 'R')
          return true;
      return false;
    }
  }
    ...
    public override void CalculateNewAge() {
      ...
      if(IsAlive) {
        if(Gender == Genders.Male) {
          ProbabilityOfDeathOtherCauses *= 1.5d;
        }
        if(Gender == Genders.Female && Age >= 2) {
