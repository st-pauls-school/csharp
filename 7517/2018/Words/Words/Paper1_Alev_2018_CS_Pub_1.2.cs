...
namespace WordsCS
{
    class Paper1Alevel2018CS
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Enter start hand size: ");
                string size = Console.ReadLine();
                if (!Int32.TryParse(size, out StartHandSize))
                    StartHandSize = -1;
        ...
        static void CalculateFrequencies(IList<string> AllowedWords)
        {
            IDictionary<char, int> dic = new Dictionary<char, int>();
            for (char c = 'A'; c <= 'Z'; c++)
                dic.Add(c, 0);
            foreach (string w in AllowedWords)
                foreach (char c in w)
                    dic[c]++;
            for (char c = 'A'; c <= 'Z'; c++)
        ...
        private static bool CheckWordIsValid(string Word, List<string> AllowedWords)
        {
            ...
            int bottom = 0;
            int top = AllowedWords.Count-1;
            int middle = top / 2;
            while(bottom < top && !ValidWord)
            {
                if (AllowedWords[middle] == Word || AllowedWords[bottom] == Word || AllowedWords[top] == Word)
                    ValidWord = true;
                else if (middle == bottom || middle == top)
                    return false;
                else
                {
                    if (AllowedWords[middle].CompareTo(Word) < 0)
                        bottom = middle;
                    else
                        top = middle;
                    middle = (bottom + top) / 2;
        ...
        static int GetScoreForWordAndPrefix(string Word, Dictionary<char, int> TileDictionary, List<string> AllowedWords)
        {
            if (string.IsNullOrEmpty(Word))
                return 0;
            return (CheckWordIsValid(Word,AllowedWords) ? GetScoreForWord(Word, TileDictionary) : 0) + GetScoreForWordAndPrefix(Word.Substring(0, Word.Length - 1), TileDictionary, AllowedWords);
        }
        ...
        private static void UpdateAfterAllowedWord(string Word, ref string PlayerTiles, ref int PlayerScore, ref int PlayerTilesPlayed, Dictionary<char, int> TileDictionary, List<string> AllowedWords)
        {
            ...
            PlayerScore = PlayerScore + GetScoreForWordAndPrefix(Word, TileDictionary, AllowedWords);
        }
