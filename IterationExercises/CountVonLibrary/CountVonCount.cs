...
namespace CountVonLibrary
{
    public class CountVonCount
    {
        ...
        public Tuple<ulong, string, bool> FindLongest(int limit, bool debug=false)
        {
            ...
            for (int i = 0; i < _numbers.Count; i++)
            {
                try
                {
                    ...
                    checked
                    {
                        trial = (ulong)_numbers[i].Item1 * (ulong)Math.Pow(10, powers);
                        trial += value;
                    }
            ...
            var rv = new Tuple<ulong, string, bool>(largest, Generate(largest), overflow);
            if(debug)
